...
Func<Task<IStorageConnection>> sqlConnectionFactory
            = () => OpenOracleSqlConnectionAsync("1234567");
...
static async Task<IStorageConnection> OpenOracleSqlConnectionAsync(string connectionString)
{
    var connection = new OracleConnection(connectionString);    
    await connection.OpenAsync();

    var connection1 = OracleConnection.Create(connectionString);
